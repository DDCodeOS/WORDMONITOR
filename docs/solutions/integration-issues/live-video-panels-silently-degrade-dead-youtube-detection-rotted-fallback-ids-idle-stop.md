...
Status: diagnosis verified against production and against the code at 618757b97b. The Defect 1 fix merged in #8155. Defect 3 has a data fix and a liveness checker on branch `fix/live-video-links`; runtime detection of streams that end later is still to come. The Defect 2 detection path was retired rather than repaired, so its fixes below are kept as history.
