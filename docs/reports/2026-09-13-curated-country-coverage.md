...
Controlled RSS tests exercise the registered server feeds, English scheduling, RSS allowlisting, browser parity, parsing, country matching, source metadata and publisher-family counting. They retain both thin-grounding and uncurated-grounding failure cases. Existing feed, provenance and development suites pass (125 tests); freeze/corpus suites pass (218 tests). Browser and API typechecks and boundary lint pass. Both existing Sources settings browser tests pass. Manual inspection confirmed the new entries at desktop and 390px mobile widths, and Guardian Pacific remained selected after closing and reopening Settings. Screenshots and exact-head CI results are recorded in the PR.
